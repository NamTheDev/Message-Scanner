...
module.exports = {
    ...
    async run(client, message) {
        ...
        try {
            ...
            const prompt = `
            Analyze the following message to see if it contains racial slurs.
            
            Message: "${message.content}"${contextMessageContent}

            Respond with "HARMFUL" if the message is harmful and should be deleted, otherwise respond with "SAFE".
            `;
            ...
            if (text === 'HARMFUL') {
                console.log(`Detected harmful message from ${message.author.tag}: "${message.content}". Deleting...`);
                ...
                if (message.member) {
                    try {
                        await message.member.timeout(timeoutDuration, 'Sending harmful content');
